...
global apez_Authenticated
global kyleB_Authenticated
global liz_Authenticated
global localClassifier
...
def getSentimentPresent(listofids):
    ...
    for id in listofids:
        ...
        auth = OAuth1(twitterAPIcredents.apez_consumerKey, twitterAPIcredents.apez_consumerSecret,
                      twitterAPIcredents.apez_oauthtoken,
                      twitterAPIcredents.apez_oauthsecret)
        requests.get(url, auth=auth)

        url1 = 'https://api.twitter.com/1.1/statuses/show.json?id={}'.format(id)
        r = requests.get(url1, auth=auth)
        responseString = r.text
        responseJSON = json.loads(responseString)
        tweet = responseJSON['text']
        returnedSentiment = SA.getSentimentOnTweet(localClassifier, tweet)
        tweets.append(tweet)
        if returnedSentiment == 'Positive':
            sentimentValues.append(1)
        else:
            sentimentValues.append(0)

    sentimentAvg = Average(sentimentValues)

    return tweets, sentimentAvg


def updateObjectsPresetData():
    ...
    for i in CandidateDataFromExistingJSON:
        if i.name == 'Mark Kelly':
            ...
            i.ECONOMYscores.append(sentiment)

            tweets = []
            tweets, sentiment = getSentimentPresent(cands[0][1])
            i.CORONAtweets.extend(tweets)
            i.CORONAscores.append(sentiment)

            tweets = []
            tweets, sentiment = getSentimentPresent(cands[0][2])
            i.HEALTHCAREtweets.extend(tweets)
            i.HEALTHCAREscores.append(sentiment)

            tweets = []
            tweets, sentiment = getSentimentPresent(cands[0][3])
            i.NATSECURITYtweets.extend(tweets)
            i.NATSECURITYscores.append(sentiment)

            tweets = []
            tweets, sentiment = getSentimentPresent(cands[0][4])
            i.CLIMATEtweets.extend(tweets)
            i.CLIMATEscores.append(sentiment)

            tweets = []
            tweets, sentiment = getSentimentPresent(cands[0][5])
            i.IMMIGRATIONtweets.extend(tweets)
            i.IMMIGRATIONscores.append(sentiment)
